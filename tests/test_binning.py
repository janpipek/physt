...
class TestNumpyBins(object):
    def test_int_behaviour(self):
        ...
        the_binning = binnings.NumpyBinning.construct(data, 10)
        assert np.allclose(the_binning.numpy_bins, np.histogram(data, 10)[1])

        the_binning = binnings.NumpyBinning.construct(data, 10, range=(0.2, 1.0))
    ...
    def test_bin_list_behaviour(self):
        ...
        the_binning = binnings.NumpyBinning.construct(data, edges)
...
class TestFixedWidthBins(object):
    def test_without_alignment(self):
        ...
        the_binning = binnings.FixedWidthBinning.construct(data, 1.0, align=False)
        assert np.allclose(the_binning.numpy_bins, [4.6, 5.6, 6.6, 7.6])

    def test_with_alignment(self):
        data = np.asarray([4.6, 7.3])
        the_binning = binnings.FixedWidthBinning.construct(data, 1.0, align=True)
        assert np.allclose(the_binning.numpy_bins, [4, 5, 6, 7, 8])

        the_binning = binnings.FixedWidthBinning.construct(data, 1.0, align=0.5)
        assert np.allclose(the_binning.numpy_bins, [4.5, 5.5, 6.5, 7.5])

        the_binning = binnings.FixedWidthBinning.construct(data, 1.0, align=1.5)
        assert np.allclose(the_binning.numpy_bins, [4.5, 5.5, 6.5, 7.5])

        the_binning = binnings.FixedWidthBinning.construct(data, 1.0, align=3.0)
        assert np.allclose(the_binning.numpy_bins, [3, 4, 5, 6, 7, 8, 9])

        the_binning = binnings.FixedWidthBinning.construct(data, 1.0, align=10)
        assert np.allclose(the_binning.numpy_bins, np.arange(0, 11))                


class TestHumanBins(object):
    def test_exact(self):
        data = np.random.rand(1000)
        the_binning = binnings.HumanBinning.construct(data, 10)
        assert np.allclose(the_binning.numpy_bins, np.linspace(0, 1, 11))

        the_binning = binnings.HumanBinning.construct(data, 9)
        assert np.allclose(the_binning.numpy_bins, np.linspace(0, 1, 11))

        the_binning = binnings.HumanBinning.construct(data, 11)
        assert np.allclose(the_binning.numpy_bins, np.linspace(0, 1, 11))
...
class TestIntegerBins(object):
    def test_dice(self):
        ...
        the_binning = binnings.IntegerBinning.construct(data)
        assert np.allclose(the_binning.numpy_bins, [0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5])

        the_binning = binnings.IntegerBinning.construct(data, range=(1, 6))
...
class TestExponentialBins(object):
    def test_data(self):
        ...
        the_binning = binnings.ExponentialBinning.construct(data, 2)
        assert np.allclose(the_binning.numpy_bins, [0.1, 1.0, 10.0])

        the_binning = binnings.ExponentialBinning.construct(data, 2, range=(1.0, 100.0))
...
class TestQuantileBins(object):
    def test_simple(self):
        ...
        the_binning = binnings.QuantileBinning.construct(data, 2)
        assert np.allclose(the_binning.numpy_bins, [0.1, 1.0, 10.0])

        the_binning = binnings.QuantileBinning.construct(data, 3)
        assert np.allclose(the_binning.numpy_bins, [0.1, 0.6, 2.2, 10.0])

    def test_qrange(self):
        data = np.asarray([0.1, 0.3, 0.4, 0.7, 1.0, 2.0, 2.6, 3.5, 10.0])
        the_binning = binnings.QuantileBinning.construct(data, 3, qrange=(0.4, 1.0))
